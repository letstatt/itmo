...
void bit_inv(char *array, int index) {
  char b = array[index / 8];
  char m = (1 << (index % 8));
  array[index / 8] = (b ^ m);
}
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    if (k > 0) {
      ...
      printf("  alloc_xor:");
      bd_print_vector(bd_sizes[k].alloc_xor, NBLK(k) / 2);
    }
  }
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_inv(bd_sizes[k].alloc_xor, blk_index(k, p) / 2);
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_inv(bd_sizes[k - 1].alloc_xor, blk_index(k - 1, p) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_inv(bd_sizes[k].alloc_xor, bi / 2);          // free p at size k
    if (bit_isset(bd_sizes[k].alloc_xor, bi / 2)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      bit_inv(bd_sizes[k].alloc_xor, bi / 2);
    }
  }
}
...
int bd_initfree_pair(int k, int bi, int buddy_delta) {
  int free = 0;
  if (bit_isset(bd_sizes[k].alloc_xor, bi / 2)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    lst_push(&bd_sizes[k].free,
             addr(k, bi + buddy_delta));  // put buddy on free list
  }
  return free;
}
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char) * ROUNDUP(NBLK(k) / 2, 8) /
         8;  // a half because of xor of neighbours
    bd_sizes[k].alloc_xor = p;
    memset(bd_sizes[k].alloc_xor, 0, sz);
    p += sz;
  }
