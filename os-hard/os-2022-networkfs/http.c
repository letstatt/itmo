...
char *char_urlencode(char *dst, char src) {
...
int fill_request(struct kvec *vec, const char *token, const char *method,
  ...
  for (int i = 0; i < arg_size; i++) {
    ...
    if (strcmp(method, "write") == 0) {
      strcat(request_buffer, arg);  // don't escape
    } else {
      urlencode(request_buffer + strlen(request_buffer), arg);
    }
  }
