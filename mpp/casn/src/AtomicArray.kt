...
class AtomicArray<E>(size: Int, initialValue: E) {
    ...
    fun set(index: Int, value: E) {
        a[index]!!.value = value // rewrite forcefully due to model test
    }
    ...
    fun cas2(index1: Int, expected1: E, update1: E, index2: Int, expected2: E, update2: E): Boolean {
        if (index1 == index2) {
            if (expected1 == expected2) {
                return cas(index2, expected2, update2) // accept last value
            } else {
...
abstract class Descriptor {
    ...
    val undecided: Boolean get() {
        return outcome.value == Outcome.UNDECIDED
    }
...
class DCSSDescriptor<A>(val a: Ref<A>, val expect: A, val update: Any?, val descriptor: CAS2Descriptor<A>) : Descriptor() {
    override fun complete() {
        outcomeCAS(if (descriptor.undecided) Outcome.SUCCESS else Outcome.FAIL)
        val replaceBy = if (success) update else expect
        a.v.compareAndSet(this, replaceBy)
    }
}
...
class CAS2Descriptor<A>(
    ...
    fun DCSS() : Boolean {
        val descriptor = DCSSDescriptor(b, expectB, this, this)
